...
constexpr uint16_t GATE_DELAY = 10 * 1000; // 게이트 한번 올리고 닫기 시간
...
// --- 게이트 상태 관리 함수 ---
void state_gate() {
  // 주차가 가능하고 바깥쪽 센서가 감지가 될 때 게이트 열고 10초 후 닫힘
  if (ss.is_parking_available && ss.is_on_outside_sensor) {
    set_gate('O');             // 게이트 열기
    ss.auto_close_gate = true; // 10초 후 닫힘
  }
  // 주차가 이미 되어있었는데 안쪽 센서가 감지가 안될 때 게이트 열고 10초 후
  // 닫힘
  if (!ss.is_parking_available && !ss.is_on_inside_sensor) {
    set_gate('O');             // 게이트 열기
    ss.auto_close_gate = true; // 10초 후 닫힘
  }
}
